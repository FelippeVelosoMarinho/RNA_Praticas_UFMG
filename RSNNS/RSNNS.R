...
N <- 200
x <- runif(N, 0, 2*pi)
y <- sin(x) + rnorm(N, 0, 0.1) # sen
#y <- cos(x) + rnorm(N, 0, 0.1) # cos

plot(x, y, col = "blue", xlim = c(0, 2*pi), ylim = c(-1, 1))

# Função de treinamento
treinar_mlp <- function(x, y) {
  rede <- mlp(x, y, size = 20, maxit = 4000, initFunc = "Randomize_Weights",
