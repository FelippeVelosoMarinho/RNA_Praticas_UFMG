...
for(epoch in 1:epochs) {
  for(i in 1:length(x_train)) {
    ...
    hidden_output <- tanh_activation(hidden_input) # ativação da camada escondida
    
    output_input <- hidden_output %*% w2 + b2
    output <- tanh_activation(output_input) # saída final da rede neural
    
    # Cálculo do erro
    error <- y_train[i] - output
    
    # Backpropagation
    output_delta <- error * tanh_derivative(output) # Calculo do delta da camada de saída, que nos diz o quanto temos que ajustar
    ...
    hidden_delta <- hidden_error * tanh_derivative(hidden_output) # Cálculo do delta da camada escondida, que nos diz o quanto precisamos ajustar
...
for(i in 1:length(x_test)) 
{
  ...
  output_input <- hidden_output %*% w2 + b2
  predictions[i] <- tanh_activation(output_input)
}
